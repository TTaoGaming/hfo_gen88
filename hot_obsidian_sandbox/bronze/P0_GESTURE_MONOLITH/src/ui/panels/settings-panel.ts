...
export class SettingsPanel implements PanelFactory {
  ...
  private buildGUI(): void {
    ...
    actions.add({ startLive: () => this.callbacks.onStartLive?.() }, 'startLive').name('📹 Start Live Webcam');
    actions.add({ stopLive: () => this.callbacks.onStopLive?.() }, 'stopLive').name('⏹ Stop Live Webcam');
