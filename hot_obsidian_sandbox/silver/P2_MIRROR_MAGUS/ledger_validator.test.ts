...
const HEARTBEAT_SCHEMA = z.object({
    ...
    resonance_signature: z.string().startsWith('KǍN-WATER-CHANT'),
    prev_hash: z.string().length(64),
    hash: z.string().length(64)
});

const PORT2_ENTRY_SCHEMA = z.object({
    ...
    mantra: z.string(),
    artifact_id: z.string(),
    resonance_signature: z.string().startsWith('KǍN-WATER-'),
...
describe('Mirror Magus (Port 2) - Ledger Validation', () => {
    it('should validate the heartbeat and all 64 entries', () => {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const result = PORT2_ENTRY_SCHEMA.safeParse(entry);
            if (!result.success) {
