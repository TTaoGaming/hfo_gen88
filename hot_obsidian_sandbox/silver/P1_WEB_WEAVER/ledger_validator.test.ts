...
const HEARTBEAT_SCHEMA = z.object({
    ...
    resonance_signature: z.string().startsWith('GÈN-MOUNTAIN-CHANT'),
    prev_hash: z.string().length(64),
    hash: z.string().length(64)
});

const PORT1_ENTRY_SCHEMA = z.object({
    ...
    mantra: z.string(),
    artifact_id: z.string(),
    resonance_signature: z.string().startsWith('GÈN-MOUNTAIN-'),
    interface: z.string(),
...
describe('Web Weaver (Port 1) - Ledger Validation', () => {
    it('should validate the heartbeat and all 64 entries', () => {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const result = PORT1_ENTRY_SCHEMA.safeParse(entry);
            if (!result.success) {
    ...
    describe('Mutation Resistance', () => {
        it('should detect a Gross Mutation', () => {
            const validLine = fs.readFileSync(LEDGER_PATH, 'utf8').split('\n')[1];
            const entry = JSON.parse(validLine);
            delete (entry as any).hfo_memory;
            expect(PORT1_ENTRY_SCHEMA.safeParse(entry).success).toBe(false);
        });
    });
});
