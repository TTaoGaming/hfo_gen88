...
const HEARTBEAT_SCHEMA = z.object({
    ...
    resonance_signature: z.string().startsWith('XÙN-WIND-CHANT'),
    prev_hash: z.string().length(64),
    hash: z.string().length(64)
});

const PORT3_ENTRY_SCHEMA = z.object({
    ...
    mantra: z.string(),
    artifact_id: z.string(),
    resonance_signature: z.string().startsWith('XÙN-WIND-'),
    delivery: z.string(),
...
describe('Spore Storm (Port 3) - Ledger Validation', () => {
    it('should validate the heartbeat and all 64 entries', () => {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const result = PORT3_ENTRY_SCHEMA.safeParse(entry);
            if (!result.success) {
