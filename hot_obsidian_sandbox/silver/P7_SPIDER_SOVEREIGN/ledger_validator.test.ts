...
const HEARTBEAT_SCHEMA = z.object({
    ...
    resonance_signature: z.string().startsWith('QIÁN-HEAVEN-CHANT'),
    prev_hash: z.string().length(64),
    hash: z.string().length(64)
});

const PORT7_ENTRY_SCHEMA = z.object({
    ...
    mantra: z.string(),
    artifact_id: z.string(),
    resonance_signature: z.string().startsWith('QIÁN-HEAVEN-'),
    decision: z.string(),
...
describe('Spider Sovereign (Port 7) - Ledger Validation', () => {
    it('should validate the heartbeat and all 64 entries', () => {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const result = PORT7_ENTRY_SCHEMA.safeParse(entry);
            if (!result.success) {
