...
const HEARTBEAT_SCHEMA = z.object({
    ...
    resonance_signature: z.string().startsWith('ZHÈN-THUNDER-CHANT'),
    prev_hash: z.string().length(64),
    hash: z.string().length(64)
});

const RESONANT_ENTRY_SCHEMA = z.object({
    ...
    mantra: z.string(),
    artifact_id: z.string(),
    resonance_signature: z.string().startsWith('ZHÈN-THUNDER-'),
    grudge: z.string(),
...
const LEDGER_PATH = path.resolve(__dirname, './BLOOD_BOOK_OF_GRUDGES.jsonl');

describe('The Resonant Blood Book of Grudges (Port 4) - Schema Enforcement', () => {
    it('should validate the heartbeat and all 64 entries', () => {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const result = RESONANT_ENTRY_SCHEMA.safeParse(entry);
            if (!result.success) {
